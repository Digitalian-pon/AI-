...
import { AppStatus, AppMode, GenerationStep, VideoModel } from './types';
import { generateAnimationVideo, fileToBase64, generateLyrics, generateTheme, generateImagePrompt, generateImage, generateAnimationPrompt, generateLipSyncVideo } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const [hasAutoGeneratedImagePrompt, setHasAutoGeneratedImagePrompt] = useState<boolean>(false);
  const [hasAutoGeneratedAnimPrompt, setHasAutoGeneratedAnimPrompt] = useState<boolean>(false);

  const getFriendlyErrorMessage = (error: unknown): string => {
    if (!(error instanceof Error)) {
      return '不明なエラーが発生しました。コンソールで詳細を確認してください。';
    }

    const message = error.message.toLowerCase();
    
    if (message.includes('quota') || message.includes('resource_exhausted')) {
      return 'APIの利用上限に達したようです。Google AI Studio (aistudio.google.com) で、APIキーの利用状況や支払い設定を確認してください。';
    }
    
    if (message.includes('api_key') && (message.includes('invalid') || message.includes('not found'))) {
      return 'APIキーが無効か、設定されていないようです。環境変数に有効なAPIキーが設定されているか確認してください。';
    }
  ...
  const handleGenerateVideo = useCallback(async () => {
    ...
    try {
      ...
      let videoUrl;
      // Use the dedicated Lipdub API function when lip sync is enabled and audio is provided
      if (lipSync && audioFile) {
        videoUrl = await generateLipSyncVideo(finalPrompt, imageBase64, imageMimeType, audioFile, videoModel);
      } else {
        // Fallback to standard animation
        // If lip sync was checked but no audio was provided, add the basic prompt enhancement.
        if (lipSync) {
            finalPrompt += ", the character is singing passionately, with mouth movements synced to the rhythm of a song.";
        }
        videoUrl = await generateAnimationVideo(finalPrompt, imageBase64, imageMimeType, videoModel);
      }
  ...
  }, [imageFile, generatedImages, selectedImageIndex, motionPrompt, audioFile, lipSync, cameraWork, effects, videoModel]);
  ...
  const handleReset = () => {
    ...
    setHasAutoGeneratedImagePrompt(false);
    setHasAutoGeneratedAnimPrompt(false);
    ...
    <div className="text-center">
        ...
        <div className="grid md:grid-cols-2 gap-6">
            ...
            <button onClick={() => onSelectMode(AppMode.UPLOAD)} className="bg-gray-700/80 p-8 rounded-2xl hover:bg-pink-900/50 border border-gray-600 hover:border-pink-500 transition-all transform hover:-translate-y-1">
                ...
                <p className="text-xs text-pink-300 mt-2 font-semibold">Suno AI や Midjourney で作成した素材に最適です！</p>
            </button>
        </div>
    </div>
  );
  ...
  const GenerateFlow = () => {
    ...
    useEffect(() => {
        if (step === GenerationStep.IMAGE && generatedLyrics && !hasAutoGeneratedImagePrompt) {
            handleGenerateImagePrompt();
            setHasAutoGeneratedImagePrompt(true);
        }
    }, [step, generatedLyrics, hasAutoGeneratedImagePrompt]);

    useEffect(() => {
        if (step === GenerationStep.ANIMATION && generatedLyrics && !hasAutoGeneratedAnimPrompt) {
            handleGenerateAnimationPrompt();
            setHasAutoGeneratedAnimPrompt(true);
        }
    }, [step, generatedLyrics, hasAutoGeneratedAnimPrompt]);
            ...
            case GenerationStep.MUSIC:
                return (
                    <div>
                        ...
                        <FileUpload label="完成した楽曲ファイルをアップロード" acceptedTypes="audio/mpeg,audio/wav,audio/x-wav" file={audioFile} onFileChange={(file) => { setAudioFile(file); if (file) setStep(GenerationStep.IMAGE); }} isAudio={true} />
                    </div>
                );
        ...
        <div>
          ...
          <div className="flex bg-gray-600 rounded-lg p-1">
            <button
              ...
              className={`flex-1 py-1 text-sm font-semibold rounded-md transition-colors ${videoModel === 'veo-3.1-fast-generate-preview' ? 'bg-purple-600 text-white' : 'text-gray-300 hover:bg-gray-500'}`}
            >
              高速
            </button>
            <button
              onClick={() => setVideoModel('veo-3.1-generate-preview')}
              className={`flex-1 py-1 text-sm font-semibold rounded-md transition-colors ${videoModel === 'veo-3.1-generate-preview' ? 'bg-purple-600 text-white' : 'text-gray-300 hover:bg-gray-500'}`}
            >
              高品質
            </button>
          </div>
  ...
  const renderLipSyncToggle = () => (
    <div className="mt-4">
        ...
        <p className="text-xs text-gray-500 mt-2">オンにすると、AIが楽曲に合わせた自然な口の動きを生成しようと試みます。より良い結果を得るために、プロンプトでも口の動きを具体的に指示することをお勧めします。</p>
    </div>
  );
